...
def main(method: str, dataset_dir, model_name_or_path: str, device):
    dataset_name = dataset_dir.rsplit("/", 1)[-1]
    print(f"dataset_name: {dataset_name}")
    r_s = 3
    wandb.init(project=f'clip_{dataset_name}_loss', name=method)
    ...
    if method.lower() != "fullft":
        config = get_lora_config(method, random_seed=r_s)
    ...
    if method.lower() == "frod":
        ...
        optimizer = AdamW(
            [
                {"params": [p for n, p in model.named_parameters() if "vera_lambda_" in n], "lr": 1e-2},
                {"params": [p for n, p in model.named_parameters() if "score" in n], "lr": 1e-4},
            ]
        )
        print("Optimizer for VeRA initialized.")
    elif method.lower() == "randlora":
        optimizer = AdamW(
            [
                {"params": [p for n, p in model.named_parameters() if "randlora_lambda" in n], "lr": 3e-4},
                {"params": [p for n, p in model.named_parameters() if "randlora_gemma" in n], "lr": 3e-4},
                {"params": [p for n, p in model.named_parameters() if "score" in n], "lr": 1e-4},
            ]
        )
        print("Optimizer for VeRA initialized.")
    ...
    for epoch in range(4):
        ...
        if eval_metric['accuracy'] > best_accuracy:
           ...
           model.save_pretrained("./result/s_0.01/clip-vit-base-" + method +"-" + dataset_name)
...
if __name__ == "__main__":
    ...
    main(args.model, args.dataset, args.model_path, device)
